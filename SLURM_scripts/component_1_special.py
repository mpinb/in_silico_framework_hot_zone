...
def setup_dask_workers(management_dir):
    ...
    sfile, sfile3 = _get_sfile(management_dir)
    for i in range(4):
        command = 'CUDA_VISIBLE_DEVICES={gpu_index} dask-worker --nthreads 1  --nprocs {nprocs} --scheduler-file={sfile} --memory-limit=100e9 &'.format(
            gpu_index=i, nprocs=1, sfile=sfile)
        print(command)
        os.system(command)
