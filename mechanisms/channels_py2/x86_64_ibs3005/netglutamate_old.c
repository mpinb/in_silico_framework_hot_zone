...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 pnt_receive_size[_mechtype] = 2;
 	...
 	ivoc_help("help ?1 glutamate_syn_old /nas1/Data_arco/project_src/in_silico_framework/mechanisms/channels/x86_64/netglutamate_old.mod\n");
...
{    _p = _pnt->_prop->param; _ppvar = _pnt->_prop->dparam;
 ...
 _tsav = t; {
   gampamax = _args[0] ;
   gnmdamax = _args[1] ;
   state_discontinuity ( _cvode_ieq + 0, & A , A + factor1 * gnmdamax * dnmda * fnmda ) ;
   state_discontinuity ( _cvode_ieq + 1, & B , B + factor1 * gnmdamax * dnmda * fnmda ) ;
   state_discontinuity ( _cvode_ieq + 2, & C , C + factor2 * gampamax * dampa * fampa ) ;
   state_discontinuity ( _cvode_ieq + 3, & D , D + factor2 * gampamax * dampa * fampa ) ;
   state_discontinuity ( _cvode_ieq + 4, & dampa , dampa * decayampa ) ;
   state_discontinuity ( _cvode_ieq + 5, & dnmda , dnmda * decaynmda ) ;
   state_discontinuity ( _cvode_ieq + 6, & fampa , fampa + facilampa ) ;
   state_discontinuity ( _cvode_ieq + 7, & fnmda , fnmda + facilnmda ) ;
