...
def filter_spike_times(spike_times, spike_times_trough):
    ...
    for x in spike_times_trough:
        aligned_creasts = spike_times[(spike_times >= x-2) & (spike_times < x)] # [y for y in spike_times if (y < x) and (y >= x-2)]
...
def burst_analysis_2(row):
    ...
    evnet_name = {0: 'singlet', 1: 'doublet', 2: 'triplet'}#, 3: 'quadruplet', 4:'quintuplet'}
    row = I.np.array(row)
    out = []
    lv = 0
    while True:
...
class EventAnalysis:
    def __init__(self, st):
        ...
        self.spike_times = I.pd.Series([d[1] for d in dummy], index = [d[0] for d in dummy])
        self.event_types = []
        self.event_extractors = {}
        self.event_db = {} # schema event_type, trial, event_time, event_parameters
...
class AnalyzeFile:
    def __init__(self, path, 
        ...
        ## extract events
        self.events = events = {e.annotations['id']: e for e in data.segments[0].events}
        ...
        # spike_times        
        _st, _st_magnitude = get_peaks_above(t, v, lim_creast) # spike times detected by creast
        _stt, _stt_magnitude = get_peaks_above(t, v * -1, lim_trough * -1) # spike times detected by trough
        ...
        self.master_pdf_row = I.pd.Series({'stim_times': self.stim_times, 'spike_times': self.spike_times})
    ...
    def get_creast_and_trough_ampltidues_by_bins(self, mode = 'zero'):
        ...
        if mode == 'zero':
            ...
            minimun_zero_bin_sst = bins[I.np.argwhere(binned_data_sst == 0).min()]
        elif mode == 'minimum':
            minimun_zero_bin_st = bins[I.np.argwhere(((binned_data_st[4:-1]-binned_data_st[5:]) < 0) | (binned_data_st[4:-1] == 0)).min() + 4]
            minimun_zero_bin_sst = bins[I.np.argwhere(((binned_data_sst[4:-1]-binned_data_sst[5:]) < 0)| (binned_data_st[4:-1] == 0)).min() + 4]
