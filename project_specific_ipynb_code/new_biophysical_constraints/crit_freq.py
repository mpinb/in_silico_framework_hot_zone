...
def setup_soma_step_with_current(cell,
                                 amplitude=None,
                                 delay=None,
                                 duration=None,
                                 dist=0):
...
def setup_crit_freq_n(cell,
    ...
    for i in range(n_stim):
        setup_soma_step_with_current(cell,
...
def modify_simulator_to_run_crit_freq_stimuli(s,
                                              delay=None,
                                              tStop=None,
                                              n_stim=None,
                                              freq_list=None,
    ...
    run_fun_crit_freq_n = partial(run_fun,
                                  T=34.0,
                                  Vinit=-75.0,
                                  dt=0.025,
                                  recordingSites=[],
    ...
    for i in range(1, len(freq_list) + 1, 1):
        s.setup.stim_response_measure_funs.append(
            [f'crit_freq{i}.hay_measure',
             param_to_kwargs(record_crit_freq)])
...
def get_spike_n_per_interval(voltage_traces, soma_threshold, delay, freq_list,
                             n_stim, a):  #v and t need to be interpolated
...
class Crit_freq:
    ...
    def NumSpike(self, voltage_traces):
        a = ([k for k, v in voltage_traces.items() if 'crit_freq' in k
             ][0].split('.'))[0][-1]
        a = int(a)
        return {
            f'crit_freq.n_spikes{a}':
                get_spike_n_per_interval(voltage_traces, self.soma_threshold,
                                         self.delay, self.freq_list,
                                         self.n_stim, a)
        }
    ...
    def Area(self, voltage_traces, returning_to_rest=2):
        ...
        v = v - baseline
               ...
               for n, x in enumerate(v[2])
               if abs(x) < returning_to_rest and n > d]
        if len(end) > 0:
            end = end[0][0]
...
def find_Crit_freq(evaluation, freq_list=None):
    ...
    if evaluation[
            'crit_freq.num_spikes_error'] > 0:  # stimuli didn't work, cannot determine crit. frequency
        ...
        area_ratio = dict([
            (freq, area_plus / area)
            for area, area_plus, freq in zip(area_list, (
                area_list[1:]), freq_list[1:])
        ])
        if max(area_ratio.values()) > 1.5:
            evaluation['crit_freq.Crit_freq'] = max(area_ratio,
                                                    key=area_ratio.get)
        else:
            evaluation['crit_freq.Crit_freq'] = float('nan')

    evaluation['crit_freq.frequency_error'] = crit_freq_error(
        evaluation['crit_freq.Crit_freq'])
...
def modify_evaluator_to_evaluate_crit_freq_stimuli(e,
                                                   freq_list=None,
                                                   delay=None,
                                                   n_stim=None,
                                                   soma_threshold=-20):
    for i in range(1, len(freq_list) + 1, 1):
        ...
        e.setup.evaluate_funs.append(
            [f'crit_freq{i}.hay_measure', cf.get, f'crit_freq{i}.features'])
    e.setup.pre_funs.append(
        put_name_of_stimulus_in_crit_freq_voltage_traces_dict)
    e.setup.finalize_funs.append(
        I.partial(check_AllStimuliWork, freq_list=freq_list))
...
def modify_combiner_to_add_crit_freq_error(c, freq_list=None):
    c.setup.append('crit_freq.error',
                   ['crit_freq.num_spikes_error', 'crit_freq.frequency_error'])
...
def visualize_crit_freq(voltage_traces,
                        delay=None,
                        tStop=None,
                        n_stim=None,
                        freq_list=None,
    ...
    if not present_mode:
        ...
        for a, b, c, i, y, freq in zip(t_list, v_list, range(1, 6), row, clm,
                                       freq_list):
        ...
        for i, f in zip(range(1, 6), freq_list):
            ...
            t = interpolate_vt(
                voltage_traces)[f'crit_freq{i}.hay_measure']['tVec']
            v = interpolate_vt(
                voltage_traces)[f'crit_freq{i}.hay_measure']['vList']
        ...
        for i in range(1, 6):
            currents[f'i{i}'] = voltage_traces[f'crit_freq{i}.hay_measure'][
                'iList'][0]
            for y in range(1, 4):
                currents[f'i{i}'] += voltage_traces[
                    f'crit_freq{i}.hay_measure']['iList'][y]

        # just traces
        fig, axes = I.plt.subplots(2,
                                   5,
                                   figsize=(25, 6),
                                   sharex=True,
                                   sharey='row',
        ...
        for t, v, i, ax_y, freq in zip(t_list, v_list, currents.keys(),
                                       range(6), freq_list):
