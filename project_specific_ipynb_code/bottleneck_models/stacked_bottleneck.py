...
class Model(nn.Module):
    ...
    def __init__(self,
        ...
        self.linear1 = nn.Linear(in_features=synaptic_input_size,
                                 out_features=bottleneck_size,
                                 bias=False)  # bias was true
        ...
        self.bottleneck_layer = nn.Linear(
            ...
            sum([bottleneck_ISI_soma, bottleneck_ISI_dend]),
        ...
        for lv in range(number_of_layers_after_bottleneck):
            layer = nn.Linear(in_features=layer_width,
                              out_features=layer_width,
                              bias=True)
        ...
        self.output_layer = nn.Linear(in_features=layer_width,
...
class CustomDataset(Dataset):
    ...
    def __init__(self, numpy_store_object, device='cpu'):
        ...
        self.arrays = [
            torch.from_numpy(numpy_store_object.load(name)[1])
...
def get_descriptor(self):
    descriptor = 'loss_' + '_'.join(
        [ln for ln, l in zip(loss_names, losses) if l])
    ...
    if bottleneck_ISI_dend:
        descriptor += 'd'
    descriptor += '__width_min_max_{}_{}_{}'.format(temporal_window_width,
                                                    t_rolling_window_min,
                                                    t_rolling_window_max)
    descriptor += '__decoder_{}x{}'.format(number_of_layers_after_bottleneck,
                                           layer_width)
    descriptor += '__L2_{}_L1_{}_TV_{}'.format(l2_lambda, l1_lambda,
...
def closure(tracking=False):
    ...
    for i in range(t_rolling_window_min, t_rolling_window_max):
        ...
        X_ISI_MCM_list = [
            SA[:, :, :,
               i:temporal_window_width + i].flatten().view(len(ind), -1),
            ISI_SOMA[:, [temporal_window_width + i]].view(len(ind), -1),
            ISI_DEND[:, [temporal_window_width + i]].view(len(ind), -1)
        ]
        desired_outputs = [
            None,  # currently not needed, AP_SOMA[:,[temporal_window_width+i]].view(len(ind),-1),
            ...
            None
        ]  # VT_DEND[:,[temporal_window_width+i]].view(len(ind),-1)]
        #t0000 = time.time()
        ...
        for lv, output in enumerate(outputs):
            desired_outputs[-1] = VT[:, lv, [temporal_window_width + i]].view(
                len(ind), -1)
    ...
    if mode == 'train':
        ...
        if total_variation_weight:
            for model in models:
                TVL = torch.pow(
                    model.linear1.weight.view(
                        bottleneck_size, n_celltypes, n_spatial_bins,
        ...
        if adjacent_models_TV:
            for a, b in all_neighbors:
                loss = torch.pow(flat_parameters[a] - flat_parameters[b],
                                 2).sum() * adjacent_models_TV
                loss.backward()
    #if tracking:
...
def log(detailed_log=False, print_=False):
    ...
    loss_collection.append(
        (mode + '_loss', epoch, batch, float(loss.cpu().detach().numpy())))
    if print_:
    ...
    if detailed_log:
        ...
        for lv, (l, n, e) in enumerate(zip(losses, loss_names, evaluations)):
            ...
            if e == 'AUROC':  # for sonmatic and dendritic acition potential prediction (binary)
                AUC = roc_auc_score(desired_outputs[lv].flatten(),
                                    model_out[:, i].flatten())
                loss_collection.append(
                    (mode + '_' + n + '_' + e, epoch, batch, AUC))
            elif e == 'pearson':  # for sonmatic and dendritic membrane potential prediction (continous)
                r = pearsonr(desired_outputs[lv].flatten(),
                             model_out[:, i].flatten())[0]
                loss_collection.append(
                    (mode + '_' + n + '_' + e, epoch, batch, r))
            if print_:
                print(loss_collection[-1])
            i = i + 1
...
def show(model, savepath=None):
    ...
    fig, axes = I.plt.subplots(bottleneck_size,
                               2,
                               gridspec_kw={'width_ratios': [70, 1]},
                               figsize=(6, 4 * bottleneck_size),
                               dpi=150)
    axes = axes.reshape(bottleneck_size, 2)
    for lv in range(bottleneck_size):
        weights1 = weights[lv, :]
        weights1 = weights1.reshape(n_celltypes, n_spatial_bins,
                                    temporal_window_width)[:, sorted_index, :]
        scaling = max(weights1.max(), -weights1.min())
        weights_for_fig = I.np.concatenate(
            [I.np.transpose(weights1[0]),
             I.np.transpose(weights1[1])], axis=0)
        im = axes[lv, 0].imshow(weights_for_fig,
                                vmin=-scaling,
                                vmax=scaling,
                                cmap='seismic',
                                interpolation='none')
...
def set_data():
    ...
    ISI_SOMA = ISI_SOMA[ind, 445 - 80:445 + 60].float().to(device).view(
        len(ind), -1)
    ISI_DEND = ISI_DEND[ind, 445 - 80:445 + 60].float().to(device).view(
        len(ind), -1)
    AP_SOMA = AP_SOMA[ind,
                      445 - 80:445 + 60].float().to(device).view(len(ind), -1)
    AP_DEND = AP_DEND[ind,
                      445 - 80:445 + 60].float().to(device).view(len(ind), -1)
...
def save():
    if savedir is not None:
        with open(
                savedir.join('model__epoch_{}__batch_{}'.format(epoch, batch)),
                'wb') as f:
...
def train():
    ...
    for epoch in range(n_epochs):
        ...
        mode = 'train'
        clear_tracking()
        for batch, ind in enumerate(I.utils.chunkIt(
                ind_train, n_batches)):  # set global variable ind
            t00 = time.time()
            ...
            loss = closure(tracking=True
                          )  # get loss (if mode==train: accumulate gradients)
            t00 = time.time()
        ...
        #show(savedir)
        save()  # save model
        print('epoch {} of {} took {}s. n_batches: {}'.format(
            epoch, n_epochs,
            time.time() - t0, n_batches))
