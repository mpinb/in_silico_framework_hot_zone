...
class BottleneckANNSimulator:
    ...
    def __init__(self, model_soma_vt, model_soma_ap, model_dend_vt, model_dend_ap, spatial_bins_df=None, 
                 outdir = '', network_paramfile=None):
        '''
        Args:
        ...
        self.outdir = outdir
        self.network_param_file = self.__get_network_from_network_param_file(network_paramfile)
    ...
    def __get_network_from_network_param_file(self,network_paramfile):
        ...
        if network_paramfile is not None:
            network_paramfile = load_param_file_if_path_is_provided(network_paramfile)
            scp.load_NMODL_parameters(network_paramfile)
            network_param_file = network_paramfile.network
        return network_param_file
            
    def __create_synaptic_input_matrix_from_network_param_file(self,network_paramfile=None, spatial_bins_df=None):
        '''To be implemented!'''
        self.network_param_file = self.__get_network_from_network_param_file(network_paramfile)
        ...
        input_activity = np.array([])# Trials, activity populations, spatial bins, time bins
        # TODO: sample from self.network_param_file
        return input_activity
    
    def run_new_simulations(self, tStop = 60, soma_isi = 100, dend_isi= 100, input_activity=None):
        ...
        for t in range(tStop):
            synapse_activations = input_activity[:,:,:,t:self.temporal_history_window+t].flatten()
            synapse_activations = (synapse_activations.view(len(trials),-1)).float()
    
            prediction_soma_vt = self.model_soma_vt.forward([synapse_activations, soma_isi, dend_isi]) 
            prediction_dend_vt = self.model_dend_vt.forward([synapse_activations, soma_isi, dend_isi]) 
            # check if its between 0 and 1, otherwise use torch.sigmoid:
            prediction_soma_ap = self.model_soma_ap.forward([synapse_activations, soma_isi, dend_isi]) 
            prediction_dend_ap = self.model_dend_ap.forward([synapse_activations, soma_isi, dend_isi]) 
            ...
            soma_aps = prediction_soma_ap < torch.from_numpy(np.random.rand(1000))
            dend_aps = prediction_dend_ap < torch.from_numpy(np.random.rand(1000))
            soma_isi[soma_aps] = 0
