from isf_data_base.isf_data_base import DataBase, MdbException
...
from tests.test_isf_data_base.context import FreshlyInitializedMdb
import pandas
...
def test_can_detect_self_as_dumper(empty_db):
    '''dumper string should be the same if it is determined
    post hoc (by providing the path to an already existing folder)
    or from the module reference directly.'''
    empty_db.setitem('test', 1, dumper='self')
    s1 = empty_db._detect_dumper_string_of_existing_key('test')
    assert s1 == 'self'
...
def test_metadata_update(empty_db):
    ...
    assert empty_db.metadata['test']['dumper'], 'self'
    ...
    metadata_db_path = os.path.join(empty_db.basedir, 'metadata.db')
    assert os.path.exists(metadata_db_path)
    os.remove(os.path.join(empty_db.basedir, 'metadata.db'))
    ...
    assert db.metadata['test']['dumper'] == 'self'
...
def test_can_set_items_using_different_dumpers(empty_db):
    empty_db.setitem('test_self', 1, dumper='self')
...
def test_overwriting_an_item_deletes_old_version(empty_db):
    ...
    def count_number_of_subfolders(key):
        ...
        folders = [f for f in os.listdir(empty_db.basedir) if key in f]
        return len(folders)
...
def test_check_if_key_exists_can_handle_str_and_tuple_keys(empty_db):
    ...
    assert not empty_db.check_if_key_exists('b')


def test_dumper_can_be_updated_and_metadata_is_adapted(empty_db):
    empty_db.setitem('a', 1, dumper='self')
    m = empty_db.metadata['a']
    assert m['dumper'] == 'self'
    empty_db.change_dumper('a', to_pickle)
    m = empty_db.metadata['a']
