"""
Test model_data_base_v2
this version of MDB should not have msgpack, pickle or sqlite backend
"""
...
import model_data_base.IO.LoaderDumper.pandas_to_paruet
...
def test_get_dumper_string_by_dumper_module(empty_mdb):
    ...
    s1 = IO.LoaderDumper.get_dumper_string_by_dumper_module(pandas_to_paruet)
    s2 = 'pandas_to_paruet'
    assert s1 == s2
...
def test_metadata_update(empty_mdb):
    ...
    assert empty_mdb.metadata['test']['version'] == get_versions(
    ...
    assert empty_mdb.metadata['test2'][
        'metadata_creation_time'] == 'together_with_new_key'
