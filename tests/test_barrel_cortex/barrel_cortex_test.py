...
def test_transform_point():
    assert (transform_point(transform_point([0, 0, 0], 1, 0, 0, 'C2'), -1, 0, 0,
                            'C2') == [0, 0, 0]).all()
...
def test_move_hoc_absolute():
    ...
    with utils.mkdtemp() as d:
        ...
        np.testing.assert_array_almost_equal(get_soma_centroid(outpath),
                                             [0, 0, 0])
...
def test_move_hoc_to_hoc():
    ...
    with utils.mkdtemp() as d:
        ...
        outpath2 = os.path.join(d, 'hoc2.hoc')
        move_hoc_to_hoc(hoc_in=hocpath,
                        hoc_reference=outpath1,
                        outpath=outpath2)
...
def test_get_distance_below_L45_barrel_surface():
    np.testing.assert_almost_equal(get_distance_below_L45_barrel_surface(
        barrel_center_points.loc['C2'], barrel='C2'),
                                   0,
                                   decimal=3)
    np.testing.assert_almost_equal(get_distance_below_L45_barrel_surface(
        barrel_center_points.loc['D2'], barrel='D2'),
                                   0,
                                   decimal=3)
...
def test_get_distance_below_L45_surface():
    np.testing.assert_almost_equal(
        get_distance_below_L45_surface(barrel_center_points.loc['C2']), 0)
    np.testing.assert_almost_equal(
        get_distance_below_L45_surface(barrel_center_points.loc['D2']), 0)
...
def test_correct_hoc_depth():
    ...
    with utils.mkdtemp() as d:
        ...
        measure_fun = partial(get_distance_below_L45_barrel_surface,
                              barrel='C2')
        correct_hoc_depth(hoc_in=hocpath,
                          hoc_reference_or_depth=0,
