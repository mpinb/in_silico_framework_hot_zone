...
def data_frame_generic_small(mdb, pdf, ddf, dumper, client=None):
    ...
    clean_up(mdb)
    if client is None:
        mdb.set('test', ddf, dumper=dumper)
    else:
        mdb.set('test', ddf, dumper=dumper, client=client)
    dummy = mdb['test']
    ...
    clean_up(mdb)
    if client is None:
        mdb.set('test', ddf.set_index(0), dumper=dumper)
    else:
        mdb.set('test', ddf.set_index(0), dumper=dumper, client=client)
    dummy = mdb['test']
...
def test_dask_to_csv_small(empty_mdb, pdf, ddf):
    data_frame_generic_small(empty_mdb, pdf, ddf, dask_to_csv)
...
def test_numpy_to_npy(empty_mdb, pdf):
    ...
    def fun(x):
        clean_up(empty_mdb)
        empty_mdb.set('test', x, dumper=numpy_to_npy)
...
def test_reduced_lda_model(empty_mdb):
        ...
        empty_mdb.set('rm', Rm, dumper=reduced_lda_model)
        ...
        # is functional
        Rm_reloaded.plot()
        empty_mdb.set('rm2', Rm_reloaded, dumper=reduced_lda_model)
