...
from .IO.LoaderDumper import to_cloudpickle, just_create_folder, just_create_mdb, shared_numpy_store, get_dumper_string_by_dumper_module
from . import model_data_base_register
from . import MdbException
from .utils import calc_recursive_filetree, rename_for_deletion, delete_in_background, is_mdb, bcolors
...
class ModelDataBase:
    ...
    def create_sub_mdb(self, key, register = 'as_parent', **kwargs):
        ...
        for k in remaining_keys:
            parent_mdb.set(k, None, dumper = just_create_mdb, **kwargs)
    ...
    def keys(self):
        ...
        keys_ =  tuple(
            ...
            and e.name not in ["dbcore.pickle", "metadata.db", "sqlitedict.db", "Loader.json"] # mdbv1 compatibility
    ...
    def remove(self):
        ...
        def delete_and_deregister_once_deleted(dir_to_data, unique_id):
            ...
            register = model_data_base_register._get_mdb_register()
...
def get_mdb_by_unique_id(unique_id):
    mdb_path = model_data_base_register._get_mdb_register().registry[unique_id]
