...
def create_metadata(mdb):
    ...
    sim_trail_index_dask = dask.dataframe.from_pandas(sim_trail_index, npartitions = 5000)
...
def get_max_commas(paths):
    ...
    filepath_chunks = utils.chunkIt(paths, 3000) # count commas in max 300 processes at once. Arbitrary but reasonable.
...
def _build_param_files(mdb, client):
    ...
    ds = generate_param_file_hashes(mdb['simresult_path'], mdb['sim_trail_index'])
    ...
    ds = write_param_files_to_folder(df, 
...
#     with dask.set_options(get = get):
    ...
    if parameterfiles: _build_param_files(mdb, client = client)                          
    if synapse_activation: 
        _build_synapse_activation(mdb, repartition = repartition, n_chunks = n_chunks)
        if rewrite_in_optimized_format:
...
def optimize(mdb, dumper = None, select = None, get = None, repartition = False, client = None):
    '''
