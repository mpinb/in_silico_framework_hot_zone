...
def sa_to_vt_bypassing_lock(mdb_loader_dict,
                            descriptor,
                            classname,
                            sa,
    ...
    PSP_identifiers = PSP_identifiers.groupby(
        ['neuron_param_mdbpath', 'confile']).apply(lambda x: list(x.index))
...
def get_mdb_loader_dict(mdb, descriptor=None, PSPClass_name=None):
    ...
    mdb_loader_dict = {
        k: partial(load, os.path.join(mdb['PSPs'].basedir, v))
