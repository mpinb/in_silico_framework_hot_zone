...
def get_PSP_determinants_from_mdb(mdb):
    ...
    parameterfiles = get_parameterfiles_df_with_confile_and_neuron_param_path(mdb)
    return parameterfiles[['confile', 'neuron_param_mdbpath']].drop_duplicates().reset_index(drop = True)


def init(mdb, client = None, description_key = None, PSPClass = None, PSPClass_kwargs = {}):
    ...
    for index, row in pdf.iterrows():
        ...
        psp.run(client)
        psps_out.append(psp)
        keys_out.append((description_key, PSPClass.__name__, row.neuron_param_mdbpath, row.confile))
