...
#  
# @dask.delayed
# def _in_parallel_context(path, cell_path, indices, xlim = (0,1500), ylim = (-80,0)):
#     '''helper function to launch generation of images in parallel'''
#     cell = serialize_cell.load_cell_from_file(cell_path)
#     
#     for path, index in zip(path, indices):
#         lines_object = get_lines(cell, index)
#         #lines_object = cell.sections[index]
#         fig, ax = init_fig(xlim, ylim)
#         plot_lines(lines_object, ax)
#         fig.savefig(path)
#         plt.close()
#      
# def parallelMovieMaker(basedir, cell_path, indices, xlim = (0,1500), ylim = (-80,0), return_glob = False):
...
#     paths = [os.path.join(basepath, str(i).zfill(6) + '.png') for i in range(len(indices))]
#     
#     delayed_list = []
#     for x in chunkIt(zip(paths, indices), 40):
#         p, i = zip(*x)
#         dummy =  _in_parallel_context(p, cell_path, i, xlim = xlim, ylim = ylim)
#         delayed_list.append(dummy)
# 
...
#      
#      
# def cell_to_ipython_animation(cell, paramFile, xlim = None, ylim = None, tstart = 245, tend = 310, tstep = 1, \
...
# 
#     indices = [find_closest_index(cell.t, i) for i in np.arange(tstart, tend, tstep)] #in case of vardt, pic     
#     if xlim is None and ylim is None:
#         xlim, ylim = get_default_axis(range_vars[0])     
#      
#      
#     try:
#         cell_path = tempfile.mkdtemp()
#         cell_path = os.path.join(cell_path, 'cell.msgpck')
#         serialize_cell.save_cell_to_file(cell_path, cell, paramFile)
#         ret = parallelMovieMaker('asdasdasdasd', cell_path, indices, xlim = xlim, ylim = ylim, return_glob = return_glob)
#     except:
#         raise
#     finally:
#         shutil.rmtree(os.path.dirname(cell_path))
#     return ret
