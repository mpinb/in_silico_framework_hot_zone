...
class PixelObject():
    ...
    def __init__(self, extent, fig = None, array = None):
        ...
        if not fig is None and not array is None:
            raise ValueError(s)
        if not extent:
            raise ValueError("extent / axis has to be specified to generate PixelObject")
        self. extent = extent
...
def show_pixel_object(pixelObject, fig = None):
    fig.imshow(pixelObject.array, interpolation='nearest', extent = pixelObject.extent, aspect = 'auto')
    return fig
