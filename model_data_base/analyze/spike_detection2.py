...
def _helper(x):
    ...
    #print(len(spikes))
    return pd.Series(dict(st = [x for lv, x in enumerate(spikes)]))

def spike_detection(ddf):
    #print(type(mdb['voltage_traces']))
#     return mdb['voltage_traces'].apply(_helper, axis = 1)
    dummy = ddf.apply(_helper, axis = 1, meta = 'object').compute(get = multiprocessing_scheduler)
    return dummy.apply(lambda x: pd.Series({lv: x.st[lv] for lv in range(len(x.st))}), axis = 1)
...
def spike_detection2(ddf):
    dummy = dask.compute(*map(dask.delayed(lambda x: x.apply(_helper2, axis = 1)), ddf.to_delayed()), get = multiprocessing_scheduler)
