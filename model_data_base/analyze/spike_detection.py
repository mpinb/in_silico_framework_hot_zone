...
def _helper(x, threshold = 0):
    ...
    t = x.index.values
    values = x.values
    spikes = simple_spike_detection(t, values, mode = 'regular', threshold = threshold)
    #print(len(spikes))
    return pd.Series({lv: x for lv, x in enumerate(spikes)})

def spike_detection(ddf, get = None, threshold = 0):
    ...
    dummy = dask.compute(*list(map(dask.delayed(lambda x: x.apply(fun, axis = 1)), ddf.to_delayed())), get = get)
