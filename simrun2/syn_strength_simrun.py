    ...
    #    gExRange = [1.0]
        ...
        for gEx in gExRange:
            ...
            for preSynCellID in nwMap.connected_cells[cellType]:
                ...
                traceFName = dirName + '/AMPA_gmax_%.2f_NMDA_gmax_%.2f_celltype_%s_presyn_cell_%d_vm.csv' % (gAMPA, gNMDA, cellType, cellID)
            ...
            print '-------------------------------'
            summaryFilename = dirName + '/AMPA_gmax_%.2f_NMDA_gmax_%.2f_celltype_%s_vmax.csv' % (gAMPA, gNMDA, cellType)
...
#    gInhRange = [0.8+i*0.4 for i in range(3)]
    ...
    for gGABA in gInhRange:
        ...
        somaT, somaV, = [], []
        print 'testing %d %s cells' % (totalNrCells, cellType)
        cellID = 0
        for preSynCellID in nwMap.connected_cells[cellType]:
            print 'testing cell %d  (%d of %d)' % (preSynCellID, cellID+1, totalNrCells)
...
#                gVec.resize(0)
            ...
            traceFName = dirName + '/GABA_gmax_%.2f_presyn_cell_%d_vm.csv' % (gGABA, cellID)
            write_sim_results(traceFName, t, vmSoma)
