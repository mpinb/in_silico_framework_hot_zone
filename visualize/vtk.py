...
def write_vtk_pointcloud_file(out_name = None, out_dir='.', points=None, scalar_data=None, scalar_data_names=None):
    '''
    ...
    if scalar_data is not None:
        ...
        assert len(scalar_data.shape) == 2, "Please pass scalar data as a 2D array, where each element defines some type of scalar data per point."
        assert len(scalar_data_names) == scalar_data.shape[0], "You did not pass as many names as scalar data arrays. scalar data: {}, amount of names: {}".format(scalar_data.shape[0], len(scalar_data_names))
    ...
    with open(os.path.join(out_dir, out_name)+".vtk", "w+", encoding="utf-8") as of:
