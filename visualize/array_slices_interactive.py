...
def slider_plot_helper(array, x, xmin, xmax, ymin, ymax, wished_binsize = None, axis = 0):
    #for other axis: shuffle arguments
    if axis == 1:
        return slider_plot_helper(array.T, x, ymin, ymax, xmin, xmax, wished_binsize = wished_binsize, axis = 0)
    slice_ = get_slice_by_x_value(array, xmin, xmax, x)
    if wished_binsize is None:
        current_binsize = (ymax - ymin) / float((len(slice_))) #set wished binsize to current binsize

    effective_binsize, newbins, newslice = rebin_slice(slice_, ymin, ymax, wished_binsize)
...
def set_everything_up(pixelObjects, axis = 0, normalize_factors = None):
        ...
        @interact(x = (xmin, xmax, 1), wished_binsize = (0,10,.1), xmin = (ymin, ymax), xmax = (ymin, ymax))
        def fun2(x = 261, wished_binsize = 1, xmin = ymin, xmax = ymax):
            fig = plt.figure()
            hist = make_plots(pixelObjects, x, wished_binsize = wished_binsize, axis = axis, normalize_factors = normalize_factors)
