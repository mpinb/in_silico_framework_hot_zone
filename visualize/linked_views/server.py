...
class LinkedViewsServer:
    ...
    def __init__(self, config_file_path=None, data_folder=None):
        if (config_file_path is not None):
            ...
            self.config_file_path = Path(
                os.path.dirname(__file__)) / "defaults" / "config.json"
    ...
    def stop(self):
        ...
        print(
            "Stopping server. If operation does not terminate, try to repeatedly reopen http://127.0.0.1:{} in the browser."
            .format(self.port))
        self.stop_logging()
    ...
    def start_logging(self):
        ...
        self.logfile_handler = logging.handlers.RotatingFileHandler(
            filename=log_filename, mode='w', maxBytes=1048576)
    ...
    def init_routes(self):
        ...
        self.app.add_url_rule('/getSelections', 'getSelections',
                              self.getSelections)
        self.app.add_url_rule('/getSelectedIndices', 'getSelectedIndices',
                              self.getSelectedIndices)
        self.app.add_url_rule('/dataServer/',
                              'get_objects',
                              self.get_objects,
                              methods=['GET'])
        self.app.add_url_rule('/dataServer/<name>',
                              'delete_object',
                              self.delete_object,
                              methods=['DELETE'])
        self.app.add_url_rule('/dataServer/',
                              ...
                              self.add_object,
                              methods=['POST'])
        self.app.add_url_rule('/dataServer/getMetaData',
                              'getMetaData',
                              self.getMetaData,
                              methods=["GET", "POST"])
        self.app.add_url_rule('/dataServer/getResourceJSON',
                              'getResourceJSON',
                              self.getResourceJSON,
                              methods=["GET", "POST"])
        self.app.add_url_rule("/dataServer/getValues",
                              "getValues",
                              self.getValues,
                              methods=["GET", "POST"])
        self.app.add_url_rule("/dataServer/getDensity",
                              "getDensity",
                              self.getDensity,
                              methods=["POST"])
        self.app.add_url_rule("/dataServer/setDensityPlotSelection",
                              "setDensityPlotSelection",
                              self.setDensityPlotSelection,
                              methods=["POST"])

    def index(self):
        if (self.vaex_df):
            return "vaex df columns: {}".format(
                [name for name in list(self.vaex_df.columns)])
        else:
    ...
    def add_object(self):
        ...
        self.objects = delete_object_by_property(self.objects, "name",
                                                 new_object["name"])
                ...
                if (self.vaex_df is not None):
                    meta_data.append({
                        "name":
                            "vaex_df",
                        "num_rows":
                            self.vaex_df.shape[0],
                        "columns":
                            self.vaex_columns,
                        "data_ranges":
                            self.
                            vaex_data_ranges  #'get_data_ranges_vaex(self.vaex_df)                
                    })
    ...
    def getDensity(self):
        ...
        if request.method == "POST":
            if request.data:
                ...
                if (format == "count"):
                    values = df.count(binby=binbycols,
                                      shape=density_grid_shape,
                                      selection=self.active_selection,
                                      limits=data_ranges).astype(float)
                    values[values == 0] = np.nan
                elif (format == "min"):
                    values = df.min(value_column,
                                    binby=binbycols,
                                    shape=density_grid_shape,
                                    selection=self.active_selection,
                                    limits=data_ranges)
                    ...
                    values = df.mean(value_column,
                                     binby=binbycols,
                                     shape=density_grid_shape,
                                     selection=self.active_selection,
                                     limits=data_ranges)
                elif (format == "median"):
                    values = df.median_approx(value_column,
                                              binby=binbycols,
                                              shape=density_grid_shape,
                                              selection=self.active_selection,
                                              limits=data_ranges)
    ...
    def computeSelection(self, return_indices=True):
        if ("global" in self.selections):
            ...
            for idx, range_i in enumerate(ranges):
                limit_i = [(range_i[0][0], range_i[0][1]),
                           (range_i[1][0], range_i[1][1])]
                self.vaex_df.select_rectangle(self.vaex_df[col1],
                                              self.vaex_df[col2],
                                              limit_i,
                                              name="selection_global",
                                              mode="or")
            self.active_selection = "selection_global"

            if (return_indices):
                df_selected_indices = self.vaex_df.evaluate(
                    self.vaex_df["row_index"], selection="selection_global")
    ...
    def getSelectedIndices(self):
        ...
        if (df_selected_indices is None):
            ...
            return "number of selected rows: {}".format(
                df_selected_indices.shape[0])
...
if __name__ == "__main__":
    ...
    def append_PCA(df, columns, descriptor, n_components=2):
        ...
        for component_idx in range(0, n_components):
            df_transformed.rename("PCA_{}".format(component_idx),
                                  "{}-{}".format(descriptor, component_idx + 1))
