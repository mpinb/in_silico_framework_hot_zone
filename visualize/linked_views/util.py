...
def binCategoricalAttributes(dataColumn,
                             values,
                             valueId_value,
                             isArrayData=False):
    value_mask = {}
    ...
    bins = []
    for value in values:
        bins.append({"value": value, "mask": value_mask[value]})
...
def binNumericAttributes(dataColumn, firstBinStartValue, lastBinStartValue,
                         step):
    bins = []
    ...
    while (currentStartValue <= lastBinStartValue):
        ...
        mask_current = (dataColumn > currentStartValue) & (dataColumn
                                                           <= currentEndValue)
    ...
    bins.append({"value": "other", "mask": ~mask_binned})
...
def getPropertyCombinationKeys(numProperties):
    ...
    powerSet = list(
        itertools.chain.from_iterable(
            itertools.combinations(propertyIndices, r)
            for r in range(len(propertyIndices) + 1)))
...
def getQuantilesForDataVector(dataVector,
                              propertyName,
                              outFolderPlot,
                              quantileStep,
                              numBins=50):
    ...
    def getQuantileCenters():
        ...
        for k1 in range(0, quantiles.size):
            if (k1 == 0):
                ...
                center = quantiles[k1 - 1] + 0.5 * (quantiles[k1] -
                                                    quantiles[k1 - 1])
                centers.append(center)
        return centers
    ...
    plotfile = os.path.join(outFolderPlot,
                            "distribution_{}.png".format(propertyName))
    plt.savefig(plotfile, dpi=300)

    dataVectorIndexed = getQuantileIndicesForDataVector(dataVector, [-1],
                                                        quantiles)
    values = []
    valueId_value = {}
...
def getQuantilesForLargeDataVector(dataVector,
                                   propertyName,
                                   outFolderPlot,
                                   quantileStep,
                                   numBins=50,
                                   plot_quantileIdxMin=-1,
                                   plot_quantileIdxMax=-1,
                                   useLog_x=False,
                                   useLog_y=False):
    quantileSteps = getQuantileSteps(quantileStep)
    print("quantile steps", quantileSteps)
...
def getParamsCols():
    params_py2 = [
        'ephys.CaDynamics_E2.apic.decay', 'ephys.CaDynamics_E2.apic.gamma',
        'ephys.CaDynamics_E2.axon.decay', 'ephys.CaDynamics_E2.axon.gamma',
        'ephys.CaDynamics_E2.soma.decay', 'ephys.CaDynamics_E2.soma.gamma',
        'ephys.Ca_HVA.apic.gCa_HVAbar', 'ephys.Ca_HVA.axon.gCa_HVAbar',
        'ephys.Ca_HVA.soma.gCa_HVAbar', 'ephys.Ca_LVAst.apic.gCa_LVAstbar',
        'ephys.Ca_LVAst.axon.gCa_LVAstbar', 'ephys.Ca_LVAst.soma.gCa_LVAstbar',
        'ephys.Im.apic.gImbar', 'ephys.K_Pst.axon.gK_Pstbar',
        'ephys.K_Pst.soma.gK_Pstbar', 'ephys.K_Tst.axon.gK_Tstbar',
        'ephys.K_Tst.soma.gK_Tstbar', 'ephys.NaTa_t.apic.gNaTa_tbar',
        'ephys.NaTa_t.axon.gNaTa_tbar', 'ephys.NaTa_t.soma.gNaTa_tbar',
        'ephys.Nap_Et2.axon.gNap_Et2bar', 'ephys.Nap_Et2.soma.gNap_Et2bar',
        'ephys.SK_E2.apic.gSK_E2bar', 'ephys.SK_E2.axon.gSK_E2bar',
        'ephys.SK_E2.soma.gSK_E2bar', 'ephys.SKv3_1.apic.gSKv3_1bar',
        'ephys.SKv3_1.apic.offset', 'ephys.SKv3_1.apic.slope',
        'ephys.SKv3_1.axon.gSKv3_1bar', 'ephys.SKv3_1.soma.gSKv3_1bar',
        'ephys.none.apic.g_pas', 'ephys.none.axon.g_pas',
        'ephys.none.dend.g_pas', 'ephys.none.soma.g_pas', 'scale_apical.scale'
    ]

    params_py3 = [
        p.replace('ephys.CaDynamics_E2', 'ephys.CaDynamics_E2_v2')
        for p in params_py2
    ]
    return params_py3
