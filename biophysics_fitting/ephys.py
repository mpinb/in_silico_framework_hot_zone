...
def BAC_caSpike_height(t,v,v_dend,ca_thresh=-55,tstim=295):
    ...
    i = (lv for lv, i in enumerate(find_crossing(v_dend, ca_thresh)[0]) if t[i] >= tstim).next()
    return hs[i]
...
def BPAPatt(t,v_dend, thresh = '+2mV',stim_onset = None):
    ...
    i = (lv for lv, i in enumerate(find_crossing(v_dend, thresh)[0]) if t[i] >= stim_onset).next()
    return h2[i]-b2
