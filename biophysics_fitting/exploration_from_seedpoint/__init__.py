...
class RW:
    ...
    def run_RW(self, selected_seedpoint, particle_id, seed=None):
        ...
        seed_point_for_exploration_pd = self.df_seeds[
            self.params_to_explore].iloc[selected_seedpoint]
        print(len(seed_point_for_exploration_pd))
        # normalize seed point parameters
        seed_point_for_exploration_normalized_pd = self._normalize_params(
            seed_point_for_exploration_pd)
        seed_point_for_exploration_normalized_selected_np = seed_point_for_exploration_normalized_pd[
            self.params_to_explore].values
        ...
        OPERATION_DIR = os.path.join(
            self.MAIN_DIRECTORY, '{}/{}'.format(selected_seedpoint,
                                                particle_id))
        ...
        iterations = [
            int(f.split('.')[0])
            for f in os.listdir(OPERATION_DIR)
        ...
        if iterations and max(iterations) > 60000:
            ...
            sys.exit(0)
        if len(iterations) == 0:
            print('So far nothing simulated, start from seedpoint',
                  selected_seedpoint)
            ...
            for iteration in iterations:
                df_path = os.path.join(OPERATION_DIR,
                                       '{}.pickle'.format(iteration))
                print('Found preexisting RW, continue from there. Iteration',
                      iteration)
                ...
                try:
                    p = df.iloc[-1][
                        self.
                        all_param_names]  # p is pandas and the full vector and unnormalized
                    break
                except IndexError:
                    print(
                        "didn't find a model inside the space, try previous iteration"
                    )

            out = []
            # set the random number generator to the latest state
            assert max(iterations) == iterations[0]
            rngn_path = os.path.join(OPERATION_DIR,
                                     '{}.pickle.rngn'.format(iterations[0]))
        ...
        while True:
            ...
            print('Position within boundaries found, step size is', step_size,
                  'Tested ', n_suggestion,
                  'positions to find one inside the box.')
            ...
            if inside:
                ...
                print(
                    'distance to initial seed point (normalized):',
                    get_vector_norm(
                        p_normalized_selected_np -
                        seed_point_for_exploration_normalized_selected_np))
            iteration += 1
    ...
    def _setup_signal_handler(self):
        ...
        def signal_handler(s, frame):
            if self.in_main_loop == False:
                print(
                    'signal received, main loop has not started, exit without attempting to save'
                )
                if s == signal.SIGUSR1:
                    sys.exit(0)
            else:
                print(
                    'signal received, main loop has started, setting flag to initiate saving'
                )
    ...
    def run_RW(self, selected_seedpoint, particle_id, seed=None):
        ...
        if len(iterations) == 0:
            ...
            for i, iteration in enumerate(iterations):
                ...
                if i == 0:
                    step_size = df.iloc[-1][
                        'step_size']  # set step size to the latest value
                df = df[df.inside]
                try:
                    p = df.iloc[-1][
                        self.
