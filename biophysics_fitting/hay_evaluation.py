...
def setup_hay_evaluator():
    ...
    with stream_to_logger(logger, 10): 
        try:
...
def get_cur_stim(stim):
    ...
    sol = h.mc.get_sol()
    stim_count = len(sol)
    return {sol.o(cur_stim).get_name().s: cur_stim for cur_stim in range(stim_count)}[
        stim
    ]
...
def hay_evaluate(cur_stim, tvec, vList):
    ...
    stim1 = (
        h.evaluator.stim1
    )  ### need to figure out, what this is ... seems to be unused in the distance calculator itself
    ...
    with stream_to_logger(logger, 10):
        try:
            ...
            return {
                k.s: 1000
                for k in list(h.evaluator.stimulus_feature_name_list.o(cur_stim))
            }
...
def hay_objective_function(x):
    ...
    This sometimes segfaults even when the correct mechanisms are loaded.
    The culprit seems to be somewhere in h.evaluator.evaluate_opulation(o)
    evaluate_population is trying to access a dendrite that is deleted.
    This is likelyo related to the uncommenting of setting up GACell v3 (is this a stump cell?)
...
from .hay_complete_default_setup import (
