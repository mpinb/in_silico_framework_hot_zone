...
def run_command(commands, args, cwd=None, verbose=False, hide_stderr=False):
    ...
    for c in commands:
        try:
            ...
            if e.errno == errno.ENOENT:
                continue
            if verbose:
                print(("unable to run {:s}".format(dispcmd)))
                print(e)
            return None
    else:
        if verbose:
            print(("unable to find command, tried {:s}".format((commands,))))
        return None
    ...
    if p.returncode != 0:
        if verbose:
            print(("unable to run {:s} (error)".format(dispcmd)))
        return None
    return stdout
...
def git_versions_from_keywords(keywords, tag_prefix, verbose):
    ...
    if not tags:
        ...
        if verbose:
            print(
                ("discarding '{:s}', no digits".format(",".join(refs - tags))))
    if verbose:
        print(("likely tags: {:s}".format(",".join(sorted(tags)))))
    for ref in sorted(tags):
        # sorting will prefer e.g. "2.0" over "2.0rc1"
        if ref.startswith(tag_prefix):
            r = ref[len(tag_prefix):]
            if verbose:
                print(("picking {:s}".format(r)))
            return {
                "version": r,
...
def git_pieces_from_vcs(tag_prefix, root, verbose, run_command=run_command):
    ...
    describe_out = run_command(GITS, [
        "describe", "--tags", "--dirty", "--always", "--long", "--all",
        "--match",
        "%s*" % tag_prefix
    ],
                               cwd=root)
    ...
    if "-" in git_describe:
        ...
        if not full_tag.startswith(tag_prefix):
            if verbose:
                ...
                print((fmt.format((full_tag, tag_prefix))))
