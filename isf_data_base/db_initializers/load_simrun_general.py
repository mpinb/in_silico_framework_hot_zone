...
with the simrun2 module into a DataBase.
'''
...
###########################################
from isf_data_base.IO.roberts_formats import _max_commas
...
def load_dendritic_voltage_traces_helper(db,
                                         suffix,
                                         divisions=None,
                                         repartition=None):
    assert repartition is not None
...
def _build_dendritic_voltage_traces(db, suffix_dict=None, repartition=None):
    ...
    out = load_dendritic_voltage_traces(db,
                                        suffix_dict,
                                        repartition=repartition)
...
def optimize(db,
    ...
    specified isf_data_base dircetory using an optimized format, which is much faster than csv
