...
from isf_data_base import distributed_lock
import sqlite3
import cloudpickle
import yaml
import sys
...
class SQLiteDict:
    ...
    def keys(self):
        return [
            k[0] for k in self.cursor.execute(SQLiteDict.GET_KEYS).fetchall()
        ]
...
while True:
    ...
    if setget == 'SET':
        ...
        socket.send(cloudpickle.dumps(list(dbs[path].keys())))
    else:
        raise ValueError()