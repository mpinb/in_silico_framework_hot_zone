...
class ReaderSmr:
    ...
    def __init__(self, path, analogsignal_id = 0, stim_times_channel = None, 
                 min_rel_time = None, max_rel_time = None):
        self.path = path
        ...
        if stim_times_channel:
            self._events = events = {e.annotations['id']: e for e in data.segments[0].events}
...
def read_labview_junk1_dat_files(path, scale = 100, sampling_rate = 32000):
    ...
    with open(path, 'rb') as f:
        while f.read(1) != '\x00': # skip header
            pass
        data = np.fromfile(f, dtype='>f4') # interpret binary data as big endian float32
...
#####################################################
def get_st_from_spike_times_and_stim_times(spike_times, stim_times, offset = 0, mode = 'spike_times'):
    ...
    if mode == 'spike_times':
        ...
        for lv, stim_time in enumerate(stim_times):
            values = [s - stim_time 
                      ...
                      if ((s - stim_time) >= offset) and ((s - stim_time) < stim_interval + offset)]
        ...
        for lv, stim_time in enumerate(stim_times):
            values = [s[1] 
                      ...
                      if ((s[0] - stim_time) >= offset) and ((s[0] - stim_time) < stim_interval + offset)]
            values = I.pd.Series({lv: v for lv, v in enumerate(values)})
            values.name = lv
...
class SpikeDetectionCreastTrough(object):
    ...
    def __init__(self, reader_object, 
        ...
        self.stim_times = stimulus_interval_filter(reader_object.get_stim_times(), 
                                                   stimulus_period, 
    ...
    def get_creast_and_trough_ampltidues_by_bins(self, mode = 'zero'):
        ...
        if mode == 'zero':
            minimun_zero_bin_st = bins[I.np.argwhere(binned_data_st[4:-1] == 0).min() + 4]
            minimun_zero_bin_sst = bins[I.np.argwhere(binned_data_sst[4:-1] == 0).min() + 4]
        elif mode == 'minimum':
            minimun_zero_bin_st = bins[I.np.argwhere(((binned_data_st[4:-1]-binned_data_st[5:]) < 0) | (binned_data_st[4:-1] == 0)).min() + 4]
    ...
    def show_events(self, events = 'auto', savepdf = None, showfig = True, ylim = (-5,5)):
    ...
    def get_serialize_dict(self):    
        return {'reader_object': self.reader.get_serialize_dict(),
...
def _spike_times_series_from_spike_times_dataframe(st):
    ...
    st = strip_st(st)
    dummy = [(name, get_spike_times_from_row(row)) for name, row in st.iterrows()]
    spike_times = I.pd.Series([d[1] for d in dummy], index = [d[0] for d in dummy])
    return spike_times
...
def _sta_apply_helper(spike_times, analysis_function, periods = {}):
    ...
    out['period'] = out.apply(lambda x: get_period_label_by_time(periods, x.event_time), axis = 1)
    return out
...
class STAPlugin_bursts(STAPlugin_TEMPLATE):
    ...
    def event_analysis_bursts(row, 
        ...
        if not sorted(event_maxtimes.keys()) == sorted(event_names.keys()):
            raise ValueError('keys of event_maxtimes and event_name must be the same!')
...
class STAPlugin_annotate_bursts_in_st(STAPlugin_TEMPLATE):
    ...
    def setup(self, spike_times_analysis):
        ...
        dfs = [STAPlugin_bursts.event_analysis_bursts(row.dropna(), 
                                                self.event_maxtimes, 
                                                self.event_names)
        ...
        df = I.pd.concat([fun(df.event_class) if len(df) else I.pd.Series() for df in dfs], axis = 1).T
        df.index = st.index
        self._result = df
...
class STAPlugin_ongoing(STAPlugin_TEMPLATE):
    ...
    def setup(self, spike_times_analysis):
        ...
        recording_start = max(spike_times_analysis.spike_times_object.t_start, 0)
        epsilon = 0.1
        ...
        n_spikes = [s for s in spike_times 
...
class STAPlugin_quantification_in_period(STAPlugin_TEMPLATE):
    def __init__(self,
        ...
        if not mode in ('frequency', 'count_per_trial', 'count_total'):
            raise ValueError('mode must be "frequency" or "count_per_trial" or "count_total"!')
        self.name = name
        self.source = source
        self.mode = mode
        self.t_start, self.t_end, self.period = _sta_input_checker(t_start, t_end, period)
...
class STAPlugin_spike_times_dataframe(STAPlugin_TEMPLATE):
    def __init__(self, name = 'spike_times_dataframe', source = 'spike_times', offset = 0, mode = 'spike_times'):
        self.name = name
        self.source = source
        self.offset = offset
        self.mode = mode
    ...
    def setup(self, spike_times_analysis):
        spike_times = spike_times_analysis.get(self.source)
        stim_times = spike_times_analysis.get('stim_times')
        self._result = get_st_from_spike_times_and_stim_times(spike_times, stim_times, offset = self.offset, mode = self.mode)
...
class SpikeTimesAnalysis:
    ...
    def __init__(self, spike_times_object, 
...
class VisualizeEventAnalysis:
    ...
    def plot_ISI1_vs_ISI2(self, 
                          interval_dict,
                          rp = 2.5, color_dict = I.defaultdict(lambda: 'k'),
                          ax1 = None, ax2 = None): 
        
        # color_dict = {'ongoing': 'grey', 'onset': 'r', 'sustained': 'k', 'late': 'green', None: 'white'}
        colormap = {10:'r', 20:'orange', 30:'green', 40: 'blue', 50: 'purple'}
        ...
        colors = [color_dict[get_interval(interval_dict, t)] for t in e.event_time]
...
class AnalyzeFile:
    def __init__(self, path, 
        ...
        self.sdct = SpikeDetectionCreastTrough(smr_reader, lim_creast=lim_creast,
        ...
        df['period'] = df.apply(lambda x: get_period_label_by_time(self.periods, x.event_time), axis = 1)
        self.event_df = df
    ...
    def describe(self):
        ...
        text += '\n' + 'ongoing activity: {} Hz'.format(af.get_ongoing_activity())
        text += '\n' + 'onset spike prob: {}'.format(af.get_onset_spike_probability())
        text += '\n' + 'onset spike latency: {} ms'.format(af.get_onset_latency())
        text += '\n' + 'creast / trough limit [mV]: {} / {}'.format(af.lim_creast, af.lim_trough)
        return text
    ...
    def get_df(self, groupby = ['period'], normed = False):
        ...
        column_order = ['singlet', 'doublet', 'triplet', 'quadruplet', 'quintuplet']
    ...
    def get_n_onset2(self):
        ...
        out2 = df.event_class.map({'singlet': 1, 'doublet': 2, 'triplet': 3, 'quadruplet': 4, 'quintuplet': 5})
