...
#    h.psection(sec=apicalBifurcationSec)
    ...
    print 'iclamp.onset {}'.format(delay)
    print 'iclamp.imax {}'.format(amplitude)
    print 'iclamp.tau0 {}'.format(tauRise)
    print 'iclamp.tau0 {}'.format(tauDecay)
    iclamp.onset = delay
    iclamp.imax = amplitude
    iclamp.tau0 = tauRise
    iclamp.tau1 = tauDecay
        
    print 'apical current injection: %.2f nA' % amplitude
    tVec = h.Vector()
    tVec.record(h._ref_t)
    startTime = time.time()
    scp.init_neuron_run(simParam, vardt=True)
...
def soma_apical_injection(cell, somaAmplitude, somaDelay, somaDuration, apicalBifurcationSec, apicalInjectionDistance, apicalAmplitude,\
    ...
    print 'selected apical section:'
#    h.psection(sec=apicalBifurcationSec)
    print somaAmplitude, somaDelay, somaDuration, apicalBifurcationSec, apicalInjectionDistance, apicalAmplitude,\
                          apicalDelayDt, apicalTauRise, apicalTauDecay, simParam
    print apicalBifurcationSec.name()
    somaDist = cell.distance_to_soma(apicalBifurcationSec, 0.0)
    apicalx = (apicalInjectionDistance - somaDist)/apicalBifurcationSec.L
    print 'apicalx: %.2f' % apicalx	
