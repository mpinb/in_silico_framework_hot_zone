...
def getRadiiHalfMax(image,pts,):
    ...
    for pt in pts:
        ...
        [halfmaxvalue,min_radius,avg_radius,front_contour_pt,back_contour_pt,rayim,minrayim,mincontptim, frontvalues, backvalues, front_contour_pts, back_contour_pts] = getHalfMaxRadiusAtThisPoint(image,pt)
...
def getHalfMaxRadiusAtThisPoint(im,pt):
    ...
    for j in range(NUM_RAYS):
        ...
        # collect the
        linear_profile_front = getLineProfileIndices(pt,phi,im,RAY_LEN_PER_DIRECTION_IMAGE_COORDS,front=True)
        linear_profile_back = getLineProfileIndices(pt,phi,im,RAY_LEN_PER_DIRECTION_IMAGE_COORDS,front=False)
        ...
        if len(linear_profile_front) > 5 and len(linear_profile_back) > 5:
            ...
            [fronthalfmaxpt,front_profile_values] = getHalfMaxPoint(im,linear_profile_front,pt)
            [backhalfmaxpt,back_profile_values] = getHalfMaxPoint(im,linear_profile_back,pt)
...
def getLineProfileIndices(pt,phi,image_plane,RAY_LEN_PER_DIRECTION_IMAGE_COORDS,front=True):
    profile_indices = []
    #profile_values = []
    ...
    for k in range(int(RAY_LEN_PER_DIRECTION_IMAGE_COORDS)):
        ...
        if(x_new <= 1 or y_new <= 1 or x_new >= image_plane.GetWidth() or y_new >= image_plane.GetHeight()):
            #print x_new,y_new
            break;
