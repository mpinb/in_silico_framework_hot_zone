...
class PSPs:
    ...
    def __init__(self,
        ...
        if exc_inh == 'exc':
            self.network_param = generate_ex_network_param_from_network_embedding(
                self.confile)
        elif exc_inh == 'inh':
            self.network_param = generate_inh_network_param_from_network_embedding(
                self.confile)
        self.network_params_by_celltype = split_network_param_in_one_elem_dicts(
            self.network_param)
        ...
        if exc_inh == 'exc':
            ...
            for n in self.network_params_by_celltype:
                ...
                for gEx in self.gExRange:
                    self._keys.append((celltype, gEx * self.AMPA_component,
                                       gEx * self.NMDA_component))
                    d = self._simfun(I.cloudpickle.dumps(self.neuron_param),
                                     I.cloudpickle.dumps(n),
                                     celltype,
    ...
    def get_voltage_and_timing(self,
        ...
        return get_voltage_and_timing(res,
                                      method,
                                      tStim=self.tStim,
                                      tEnd=self.tEnd)
    ...
    def plot_vt(self,
        ...
        t_baseline, v_baseline = I.np.arange(0, self.tEnd, 0.025), I.np.interp(
            I.np.arange(0, self.tEnd, 0.025), vt[0], vt[1])
...
def run_ex_synapse(cell_nw_generator,
                   neuron_param,
                   network_param,
                   celltype,
                   preSynCellID,
    ...
    if preSynCellID is not None:
        ...
        for syn in synapse_list:
            # syn.weight = {'glutamate_syn': [gAMPA, gNMDA]}
            activate_functional_synapse(syn,
                                        cell,
                                        preSynCell,
                                        synParameters,
...
def run_ex_synapses(neuron_param,
                    network_param,
                    celltype,
                    gAMPA=None,
                    gNMDA=None,
    ...
    neuron_param = I.scp.NTParameterSet(
        I.cloudpickle.loads(neuron_param).as_dict())
    network_param = I.scp.NTParameterSet(
        I.cloudpickle.loads(network_param).as_dict())
    # with I.silence_stdout:
    cell_nw_generator = get_cell_with_network(neuron_param, network_param)
    cell, nwMap = cell_nw_generator()
    somaT, somaV, = [], []
    t_baseline, v_baseline = run_ex_synapse(cell_nw_generator,
                                            neuron_param,
                                            network_param,
                                            celltype,
                                            None,
    ...
    if method == 'dynamic_baseline':
        ...
        vt = {
            k: {
                k: {
                    k: [
                        get_tMax_vMax_baseline(v[0], v[1], v[2][lv], v[3][lv],
                                               tStim, tEnd)
        ...
        vt = {
            k: {
                k: {
                    k: [
                        get_tMax_vMax(v[2][lv], v[3][lv], tStim, tEnd)
                        for lv in range(len(v[2]))
                    ]
                    for k, v in six.iteritems(v)
                }
...
def get_tMax_vMax_baseline(t_baseline, v_baseline, t, v, tStim=None, tEnd=None):
    ...
    assert tEnd is not None
    try:
        t, v = I.np.arange(0, tEnd,
                           0.025), I.np.interp(I.np.arange(0, tEnd, 0.025), t,
                                               v)
    except:
        raise RuntimeError()
    t_baseline, v_baseline = I.np.arange(0, tEnd, 0.025), I.np.interp(
        I.np.arange(0, tEnd, 0.025), t_baseline, v_baseline)
...
def get_tMax_vMax(t, v, tStim=None, tEnd=None):
    ...
    assert tStim is not None
    assert tEnd is not None
    t, v = I.np.arange(0, tEnd, 0.025), I.np.interp(I.np.arange(0, tEnd, 0.025),
                                                    t, v)
...
def calculate_optimal_g(pdf):
    mean = (pdf['EPSP_mean_measured'] -
            pdf['EPSP mean_offset']) / pdf['EPSP mean_slope']
    med = (pdf['EPSP_med_measured'] -
           pdf['EPSP med_offset']) / pdf['EPSP med_slope']
    max_ = (pdf['EPSP_max_measured'] -
            pdf['EPSP max_offset']) / pdf['EPSP max_slope']
