...
# other options: linux-aarch64/latest, osx-64/latest ...
...
always_yes: false
EOF
create_cmd="micromamba create -n isf38 python=3.8 -y"
echo "Creating the isf3_8 environment with \"${create_cmd}\""
$create_cmd

activate_cmd="micromamba activate isf38"
echo "Activating the isf38 environment with \"${activate_cmd}\""
$activate_cmd
echo $(which python)
echo $(python --version)
